...
def zadanie_70():
    ...
    # Данные задачи
    p = 0.85  # вероятность безотказной работы
    A1 = 1  # браковочное число
    risk_customer = 0.10  # риск заказчика

    # Функция для вычисления вероятности браковки
    def calculate_rejection_probability(n, p, A1):
        rejection_probability = sum(comb(n, k) * (p ** k) * ((1 - p) ** (n - k)) for k in range(A1 + 1))
        return rejection_probability

    # Поиск минимального n
    n = 1
    while True:
        P_reject = calculate_rejection_probability(n, p, A1)
        if P_reject >= risk_customer:
            break
        n += 1

    # Результат
    print(f"Минимальный объем выборки: {n}")
    print(f"Вероятность браковки для n={n}: {P_reject:.4f}")
